...
graph TD
    ...
    K --> L[5. 更新渲染解答區、各大指示面板 & 推理過程紀錄區]

    F -->|點擊自動推理按鈕| M[autoSolveStepByStep]
    M --> N[遞迴執行 updateSolveWorkspace]
    N --> O{最終可行位置唯一?}
    O -->|是| P[自動填入解答區並寫入推理紀錄 isAuto=true]
    O -->|否| Q[停止推理]
    P --> N
```
...
3. **推理過程紀錄 (`state.reasoningHistory`)**：
   - 每一筆在「解答區」中被填入的特徵值（無論來自自動推理或手動選擇），都會在 `state.reasoningHistory` 陣列中新增一筆推理紀錄，格式如下：
     ...
     ```javascript
     {
       ...
       position: 3,
       desc: "英國人 位於 第 3 個位置",
       rules: [ { id: "G1" | ruleId, desc: "..." }, ... ],
       isAuto: true | false  // 區分推理來源
     }
     ```

   - **`isAuto: true`（自動推理）**：由 `autoSolveStepByStep` 填入。系統自動計算推理依據：
     - 若特徵值在進入計算前 `remMap` 中就僅剩一個位置，依據固定為通用規則 **G1**（「特徵A 僅剩一個位置」）。
     - 否則按規則順序逐步過濾，只記錄真正縮減可行位置數量的規則。
   - **`isAuto: false`（手動推理）**：由使用者在「解答區」的下拉選單中手動選擇後觸發，`rules` 初始為空陣列，使用者可自行在紀錄區新增推理依據。

4. **通用推理規則 (General Rules)**：
   系統內建三條不屬於任何題目規則的通用推理依據，供紀錄使用：
   - **G1**：「特徵A 僅剩一個位置」— 適用於自動推理（自動套用）與手動推理（可選用）。
   - **G2**：「第 i 個位置僅能填入 特徵A」— 保留給使用者手動選用。
   - **G3**：「數獨規則 特徵A 刪去部分可能位置」— 保留給使用者手動選用。
...
- **解答區**（上方左側）：玩家填寫最終答案的表格。
  - 點擊下拉選單，可以選擇該位置對應的特徵值。
  - 已選取的特徵值會以黃色高亮顯示。
  - 每一行（類別）已被選過的值，在該行的其他位置下拉選單中會自動顯示為 `disabled`，避免重複選取。
  - 點擊「**重置**」按鈕可清空所有解答選項（同時清除所有推理過程紀錄）。

- **推理過程紀錄區**（上方中側）：記錄每一筆推理結果及其依據。
  - 每當解答區有特徵值被填入（無論自動或手動），系統就會在此區新增一筆紀錄。
  - 紀錄表格欄位說明：
    - **編號**：自動產生的推理序號，不可編輯。
    - **推理結果**：如「英國人 位於 第 3 個位置」，不可編輯，由系統根據解答自動產生。
    - **推理依據規則**：記錄此推理結果的邏輯依據，並因推理來源不同而有不同的行為：
      - **自動推理（🤖）所得結果**：推理依據由系統自動計算並以**純文字唯讀**顯示，**使用者無法修改或新增依據**。依據範圍限縮於與該特徵值直接相關的已啟用規則，以及通用規則 G1。
      - **使用者手動選擇所得結果**：推理依據初始為空，使用者可自由**新增、刪除、修改**依據。依據來源為**所有已啟用的規則**，加上以下三條通用規則：
        - **G1**：「特徵A 僅剩一個位置」
        - **G2**：「第 i 個位置僅能填入 特徵A」
        - **G3**：「數獨規則 特徵A 刪去部分可能位置」
    - **操作**：點擊紅色「垃圾桶」按鈕可刪除該筆推理紀錄，並同時清空解答區中對應的特徵值。

- **特徵值可行位置區 (規則綜合交集)**（上方右側）：顯示經過所有「已啟用規則」進行排他性與交集計算後，各特徵值最終合法的位置。
...
- **🤖 使用輔助推理 (自動推理)**：
  - 點擊解答區右上角的「**自動推理**」按鈕。
  - 系統會掃描「特徵值可行位置區」，將所有「可行位置僅剩一個且尚未填入解答區」的特徵值自動填入解答區，同時自動計算並寫入推理依據至「推理過程紀錄區」。
  - 此過程會反覆遞迴執行，直到再也沒有確定項，或解答已全部填滿為止。
  - 自動推理所產生的紀錄依據為**唯讀**，使用者無法修改，以確保自動推理的過程可追溯且完整。
